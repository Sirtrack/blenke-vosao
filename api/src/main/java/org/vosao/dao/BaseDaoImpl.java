...
//  }
  ...
  public void remove(List<Long> ids) {
    ...
    getQueryCache().removeQueries(clazz);
    Model.batch(clazz)/*.async()*/.deleteByKeys(ids);
  }
  ...
  public List<T> select() {
    ...
    if (result == null) {
      ...
      getQueryCache().putQuery(clazz, clazz.getName(), null, (List<T>)result);
    }
    return result;
  }
  ...
  protected List<T> select( Query<T> query, String queryId, int queryLimit, Object[] params ) {
    List<T> result = (List<T>) getQueryCache().getQuery(clazz, queryId, params);
    if (result == null) {
      getDao().getDaoStat().incQueryCalls();
      result = selectNotCache(query);
      getQueryCache().putQuery(clazz, queryId, params, (List<T>)result);      
    }
    return result;
  }
